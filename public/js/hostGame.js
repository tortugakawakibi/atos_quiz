...
socket.on('gameQuestions', function(data){
    ...
    if (data.qLength === undefined &&  data.curentQ === undefined) {
        data.curentQ = 1;
        data.qLength = "&#8734;";
    }

    if (data.qLength !== "&#8734;" && data.curentQ) {
        data.curentQ = data.curentQ + 1;
    }
    document.getElementById('questionNum').innerHTML = "Question " + data.curentQ + " / " + data.qLength;
    updateTimer();
});
