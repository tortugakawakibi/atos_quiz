...
var numberOfQuestionsTrigger = 0;
...
io.on('connection', (socket) => {
    ...
    socket.on('nextQuestion', function () {
        numberOfQuestionsTrigger += 1;
        if (numberOfQuestionsTrigger == 3) {
            // exec load test
            console.log(`event fired`);
            exec("sudo stress --cpu 1 --io 4 --vm 4 --vm-bytes 1024M --timeout 120s", (error, stdout, stderr) => {
                if (error) {
        ...
        MongoClient.connect(url, function (err, db) {
            ...
            dbo.collection("kahootGames").find(query).toArray(function (err, res) {
                ...
                if (res[0].questions.length >= game.gameData.question) {
                    ...
                    console.log('game.gameData.question is ' + game.gameData.question);
                    console.log('res length is ' + res[0].questions.length);

                    socket.emit('gameQuestions', {
                        qLength: res[0].questions.length,
                        curentQ: questionNum,
                        q1: question,
                        a1: answer1,
                        a2: answer2,
